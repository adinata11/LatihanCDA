...
# Pastikan Orange3 tersedia untuk bisa menjalankan prediksi model.
try:
    import Orange
...
# Dashboard realisasi anggaran dan prediksi ketercapaian.
# Data disimpan di folder `data/` dan model disimpan di folder `models/`.
# Aplikasi ini menawarkan visualisasi ringkasan, analisis detail,
# prediksi interaktif, serta prediksi batch dengan evaluasi hasil.
...
@st.cache_data
def load_data():
    # Baca dataset utama dari folder `data/`.
    # File `data/dataset.csv` berisi fitur dan label realisasi anggaran.
...
@st.cache_resource
def load_model():
    # Muat model Orange3 yang sudah disimpan pada folder `models/`.
    with open("models/Best_model.pkcls", "rb") as f:
        return pickle.load(f)


# ── Fungsi bantu prediksi Orange3 ─────────────────────────────────────────────
# Fungsi ini membangun instance Orange dengan fitur input yang diperlukan,
# lalu melakukan prediksi kelas dan probabilitas untuk label target.
def predict_orange(model, jumlah_spm, revisi_dipa, deviasi_rpd_persen,
                   skor_ikpa, tipe_satker):
    if not ORANGE_AVAILABLE:
        raise ModuleNotFoundError(
            "Orange3 tidak terpasang. Jalankan `pip install orange3` atau perbarui environment Anda."
        )
...
# Warna tema yang digunakan pada seluruh visualisasi untuk konsistensi.
COLOR_YA    = "#28a745"
COLOR_TIDAK = "#dc3545"
COLOR_BLUE  = "#4f8ef7"
PLOTLY_THEME = dict(
...
# ── Sidebar filters ───────────────────────────────────────────────────────────
# Sidebar menyediakan kontrol interaktif untuk memilih subset data berdasarkan
# kementerian, provinsi, jenis satker, jenis belanja, dan rentang pagu.
with st.sidebar:
...
with tab3:
    ...
    if not ORANGE_AVAILABLE:
        st.error(
            "Orange3 belum terpasang. Jalankan `pip install orange3` atau install melalui `requirements.txt` lalu refresh aplikasi."
        )
        st.stop()
